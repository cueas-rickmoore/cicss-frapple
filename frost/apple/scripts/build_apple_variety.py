...
if num_date_args == 0:
...
elif num_date_args in (3,4,5,6): # start date specified
    year = int(args[1])
    month = int(args[2])
    day = int(args[3])
    start_date = datetime(year,month,day)
    if num_date_args == 4:
        end_date = datetime(year,month,int(args[4]))
    elif num_date_args == 5: 
        end_date = datetime(year,int(args[4]),int(args[5]))
    ...
    print sys.argv
    errmsg = 'Invalid number of arguments (%d).' % len(args)
