...
FRAPPLE = {
    ...
    dataAvailable: function(data_type) {
        console.log(' ');
        console.log(' ');
        console.log('DATA AVAIALBLE FOR : ' + data_type);
        console.log('    telling wait widget');
        this.wait_widget.available(data_type);

        if (data_type == 'stages' || data_type == 'days') {
            index = this.stage_pending.indexOf(data_type);
            console.log('    data type index = ' + index);
            console.log('    pending : ' + this.stage_pending.length);
            if (index > -1) {
        ...
        } else if (data_type == 'mint') {
            if ('mintAvailable' in this._listeners_) { this._listeners_.mintAvailable('mintAvailable'); }
        }
        console.log(' ');
        console.log(' ');
    },
    ...
    forecast: function(data_type, view) {
        ...
        console.log("REQUEST :: FORECAST : " + data_type + " : dates : " + _view.start + " , " + _view.end);
        var fcast_view = this.dates.forecastView(_view);
        if (fcast_view) {
            console.log("           FORECAST VIEW : " + fcast_view.start + " , " + fcast__view.end);
            return this.genDataPairs(data_type, fcast_view.start, fcast_view.end);
        } else { return; }
    },

    fullview: function(data_type, view) {
        var _view; if (typeof view !== 'undefined') { _view = view; } else { _view = this.dates.view; }
        console.log("REQUEST :: FULL VIEW : " + data_type + " : dates : " + _view.start + " , " + _view.end);
    ...
    genDataPairs: function(data_type, start, end) {
        console.log("\n\n\nREQUEST :: gen data pairs : " + data_type + " : " + start + " : " + end);
        ...
        if (jQuery.type(start) === 'string') {
            console.log('    ' + this.dateToDateObj(start).getTime() + ' : ' + this.dateToDateObj(end).getTime());
    ...
    observed: function(data_type, view) {
        ...
        console.log("REQUEST :: OBSERVED : " + data_type + " : dates : " + _view.start + " , " + _view.end);
        if (jQuery.type(data_type) === 'string') { return this.genDataPairs(data_type, _view.start, _view.end);
    ...
    uploadAllData: function(requested_loc) {
        console.log("\n\n\n\nEVENT :: REQUESTED UPLOAD OF ALL DATA");
    ...
    uploadLocationData: function(loc_obj) {
        console.log("\n\n\n\nEVENT :: REQUESTED UPLOAD OF DATA FOR A LOCATION");
        var loc_obj = loc_obj;
        if (jQuery.type(loc_obj) === 'undefined') { loc_obj = this.locations.state; }
        this.dates.uploadStageDates(loc_obj);
        this.data.uploadFreezeRiskData(loc_obj);
...
function ToolDataManager(tool) {
    ...
    Object.defineProperty(this, "risk", { configurable:false, enumerable:true,
                                          ...
                                          set:function(risk_obj) {
                                              console.log("#########################################################");
                                              console.log("SETTING RISK : " + ("riskChanged" in this._listeners_));
                                              console.log("#########################################################");
    ...
    Object.defineProperty(this, "mint", { configurable:false, enumerable:true,
                                          ...
                                          set:function(mint_array) {
                                              console.log("#########################################################");
                                              console.log("SETTING MINT : " + ("mintChanged" in this._listeners_));
                                              console.log("#########################################################");
...
ToolDataManager.prototype.uploadFreezeRiskData = function (loc_obj) {
    ...
    console.log("DATA UPLOAD REQUEST :: /risk @ " + loc_obj.address);
...
function ToolDatesManager(tool) {
    ...
    Object.defineProperty(this, "days", {
        ...
        set:function(days_array) {
                console.log("#########################################################");
                console.log("SETTING DAYS IN SEASON : " + ("seasonDaysChanged" in this._listeners_));
                console.log("#########################################################");
                var date_mgr = this;
    ...
    Object.defineProperty(this, "stages", {
        ...
        set:function(stage_array) {
                console.log("#########################################################");
                console.log("UPDATING STAGES : " + ("stagesChanged" in this._listeners_));
                console.log("#########################################################");
                var date_mgr = this
...
ToolDatesManager.prototype.update = function(dates_obj) {
    console.log("#########################################################");
    console.log("UPDATING SEASON DATES : " + ("datesChanged" in this._listeners_));
    console.log("#########################################################");
    var changed = [];
    console.log('    dates_obj : ' + dates_obj);
    ...
    console.log("UPDATED LAST VALID : " + this.last_valid);
...
ToolDatesManager.prototype.uploadStageDates = function(loc_obj) {
    ...
    console.log("DATA UPLOAD REQUEST :: /stage @ " + loc_obj.address);
    var query = {location:{key:loc_obj.id, address:loc_obj.address, coords:[loc_obj.lat,loc_obj.lng]}, variety:loc_obj.variety, season:this.tool.dates.season};
    query = JSON.stringify(query);
    console.log("QUERY = " + query);
    var options = { url:url, type:'post', dataType:'json', crossDomain:true, data:query,
...
ToolDatesManager.prototype.view = function(view_type) {
    console.log('\n\ncreating view : ' + view_type);
    console.log('this._dates_.last_valid : ' + this._dates_.last_valid);
...
ToolLocationsManager.prototype.addLocation = function(new_id, new_loc) {
    console.log("LOCATIONS MANAGER :: ADD LOCATION : " + new_id);
    ...
    } else { console.log("ERROR :: ERROR :: ERROR :: " + new_id + " : errors found  : " + invalid); } 
}
...
ToolLocationsManager.prototype.init = function(locations, default_location, default_doi, default_variety) {
    ...
    console.log("FRAPPLE :: initial location : " + this._state_.id);
...
ToolLocationsManager.prototype.update = function(new_loc, fire_event) {
    ...
    if (fire_event !== false && changed && "locationChanged" in this._listeners_) {
        ...
        this._listeners_.locationChanged("locationChanged",[changed,this.state]);
    }
}
...
ToolLocationsManager.prototype.validate = function(new_loc) {
    ...
    console.log("LOCATION VALIDATE :: " + loc_obj.id + " : " + invalid.length);
...
var initializeToolManager = function() {
    ...
    FRAPPLE.dates.upload_callbacks.season = function(uploaded_obj, status_text, jq_xhr) {
        console.log('EVENT :: SEASON DATES : upload complete : dates.upload_callback');
    ...
    FRAPPLE.dates.upload_callbacks.stage = function(uploaded_obj, status_text, jq_xhr) {
        console.log('EVENT :: STAGES : upload complete : dates.upload_callback');
        FRAPPLE.dates.update(uploaded_obj.stage.dates);
        console.log("****** SETTING STAGES ARRAY !!");
    ...
    FRAPPLE.data.upload_callbacks.risk = function(uploaded_obj, status_text, jq_xhr) {
        console.log("RISK POTENTIAL data upload complete :: data :");
